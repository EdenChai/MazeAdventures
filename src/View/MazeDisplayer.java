...
public class MazeDisplayer extends Canvas
{
    ...
    private void drawMazeTiles(GraphicsContext graphicsContext, int rows, int cols, double cellHeight, double cellWidth)
    {
        ...
        try
        {
            wallImage = new Image(new FileInputStream(getImageFileNameWall())); //TODO - remove all ref for this
        }
        catch (FileNotFoundException e)
        {
        ...
        for (int i = 0; i < rows+2; i++)
        {
            for (int j = 0; j < cols+2; j++)
            {
                ...
                if (maze.getMaze()[i-1][j-1] == 1)
                {
                    ...
                    drawTheImage(tiles[0+variant],graphicsContext,cellHeight,cellWidth,x,y);
                }
                //if its a road tile
                if (maze.getMaze()[i-1][j-1] == 0)
                {
                    ...
                    if (i !=1)
                        canMoveUp = (mazeTiles[i - 2][j-1] == 0);
                    boolean canMoveDown = false;
                    if (i != rows)
                        canMoveDown =  (mazeTiles[i][j-1] == 0);
                    boolean canMoveLeft = false;
                    if (j !=1)
                        canMoveLeft =  (mazeTiles[i-1][j-2] == 0);
                    boolean canMoveRight = false;
                    if ( j != cols)
                        canMoveRight =  (mazeTiles[i-1][j] == 0);
    ...
    public void setGoalDirection()
    {
        ...
        else if (maze.getGoalPosition().getColumnIndex() >= maze.getColSize() / 2)
            direction = "left";
        else if (maze.getGoalPosition().getColumnIndex() < maze.getColSize() / 2)
            direction = "right";
        switch (direction)
    ...
    public void playMovementSound()
    {
        ...
        MediaPlayer movementSoundPlayer = new MediaPlayer((moveSounds[0+variant]));
